		...
		<div class="right" style="margin-top:7px;">
			<form action="<?php echo U('Trade/myDeal');?>" method="post">
                <select id="coninname" name="currency">
                    <?php if(is_array($culist)): $i = 0; $__LIST__ = $culist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ((isset($vo['currency_id']) && ($vo['currency_id'] !== ""))?($vo['currency_id']):""); ?>" <?php if(($c_id) == $vo['currency_id']): ?>selected="selected"<?php endif; ?>><?php echo (getCurrencynameByCurrency($vo['currency_id'])); ?>对<?php echo (getCurrencynameByCurrency($vo['trade_currency_id'])); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>    
                </select>
