...
require_once (APP_PATH . 'Common/Api/SmsApi.php');
...
class CommonController extends Controller {
    ...
    public function _initialize(){
        ...
        $arr['status'] = 5; 
        $arr['pid'] = $member_id;
        $total=M('Member')->where($arr)->count();
		...
		$bizhong = $this->getCurrencyMessageById($zhu['currency_id']);
		$where['member_id']=$_SESSION['USER_KEY_ID'];
				
    		foreach( $this->currency as $k=>$v){
    			if($v['is_line'] !=0){
    				if($v['trade_currency_id']!='0'){
    					$bi = $this->getCurrencyMessageById($v['currency_id']);
    					$where['currency_id'] = $v['currency_id'];
    					$yue_text = M('Currency_user')->where($where)->find();
    					$yue =$yue_text['num']+$yue_text['forzen_num']; 
    					$bil[] = $bizhong['new_price']*$bi['new_price']*$yue;
    				}else{
    					$bi = $this->getCurrencyMessageById($v['currency_id']);
    					$where['currency_id'] = $v['currency_id'];
    					$yue_text = M('Currency_user')->where($where)->find();
    					$yue =$yue_text['num']+$yue_text['forzen_num']; 
    					$bil[] = $bi['new_price']*$yue;
    				}
    			}
    				
    		}
		$zongjin=array_sum($bil);
		$this->assign('sum',$zongjin);
...
// 	}
    protected function  User_status(){
        ...
        if($list['status']== 3){
            session('STATUS',3);
            $this->redirect('ModifyMember/userAuthentication','',1,"<script>alert('请先进行身份证认证')</script>");
            // $this->redirect('ModifyMember/modify','',1,"<script>alert('您的账号正在审核中,请耐心等待')</script>");
            exit();
        }
        if($list['status']== 4){
            session('STATUS',4);
            $this->redirect('Safe/index','',1,"<script>alert('账号正在审核,请耐心等待')</script>");
            // $this->redirect('ModifyMember/modify','',1,"<script>alert('您的账号正在审核中,请耐心等待')</script>");
            exit();
        }
    }
    ...
    public function getCurrencyMessageById($id){
		if(S('trade_info_'.$id)){
			...
			$time=time();
			$m = M('Trade_'.$id)->Field('price,num')->order('add_time desc')->find();
			...
			//$y = M('Trade_'.$id)->Field('price')->where($where)->order('price desc')->find();
			//$z = M('Trade_'.$id)->Field('price')->where($where)->order('price asc')->find();
			$x = M('Trade_'.$id)->where($where)->Field('sum(num) as allnum,sum(money) as all_money')->find();
    ...
    public function pullMessageCount($id){
        ...
        $where['status'] = 0;
        $where['type'] = array('neq',-2);
        $count = M('Message')->Field('message_id')->where($where)->count();
        if(empty($count)){
            return 0;
        }
        return $count;
    ...
    protected function login_limit_time(){
     ...
     protected  function  getCurrencyByCurrencyId($currency_id=0){
		if (empty($currency_id)) {
     	...
     	//获取交易币种信息
     	$list= M('Currency')->field("currency_id,currency_mark,currency_digit_num,currency_logo,currency_name,is_lock_6,is_lock_7,start_time_h,start_time_m,end_time_h,end_time_m,price_up,price_down,currency_buy_fee,currency_sell_fee,trade_currency_id,is_lock,rpc_url,rpc_pwd,rpc_user,port_number,rpc_url1,rpc_pwd1,rpc_user1,port_number1,currency_all_tibi,currency_day_times,currency_limit,currency_fee_reward,currency_all_num")->where($where)->select();
		if (!empty($currency_id)) {
    		return $list[0];  	
     	}else{
     		return $list;
