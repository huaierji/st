...
class ArtController extends CommonController {
	...
	public function index(){
		...
		$list = $article->Field('article_id,title,content,position_id,add_time,status')->where($where)->order('add_time desc')->limit($page->firstRow.','.$page->listRows)->select();//时间降序排列，越接近当前时间越高
		foreach ($list as $k=>$v){
    ...
    public function details(){
		...
		if(S('art_one_'.$id)){
			$art_one = S('art_one_'.$id);
		}else{
			$art_one = $article->Field('article_id,title,content,position_id,add_time,status')->where($where)->find();//查找到单一的文章	
			//将数据库中html标签字符串化，显示
			$art_one['title']=html_entity_decode($art_one['title']);
			$art_one['content']=html_entity_decode($art_one['content']);
			S('art_one_'.$id,$art_one);
		}
