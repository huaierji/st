...
<div class="ybc_list">
	<div class="ybcoin clearfix h2_bor">
		<a href="<?php echo U('Entrust/manage');?>"><h2 class="left" style=" border-bottom:2px solid #f60;">委托管理</h2></a>
        <!-- <a href="<?php echo U('Entrust/history');?>"><h2 class="left" style=" color:#999 !important;">委托历史</h2></a> -->
		<div class="right" style="margin-top: 7px;">
			<form action="<?php echo U('Entrust/manage');?>" method="post">
                <select id="coninname" name="currency">
                    <?php if(is_array($culist)): $i = 0; $__LIST__ = $culist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ((isset($vo['currency_id']) && ($vo['currency_id'] !== ""))?($vo['currency_id']):""); ?>" <?php if(($post["currency"]) == $vo['currency_id']): ?>selected<?php endif; ?>  ><?php echo (getCurrencynameByCurrency($vo['currency_id'])); ?>对<?php echo (getCurrencynameByCurrency($vo['trade_currency_id'])); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>    
                </select>
                <select id="status" style="margin-left:0; width:80px;" name="status">
                     <option value="0" <?php if(($post["status"]) == "0"): ?>selected<?php endif; ?> >未成交</option>
                     <option value="2" <?php if(($post["status"]) == "2"): ?>selected<?php endif; ?> >已成交</option>
                     <option value="1" <?php if(($post["status"]) == "1"): ?>selected<?php endif; ?> >部分成交</option>
                     <option value="-1" <?php if(($post["status"]) == "-1"): ?>selected<?php endif; ?> >已撤销</option>
                     <option value="0,1,2,-1" <?php if(($post["status"]) == "0,1,2,-1"): ?>selected<?php endif; ?>>全部</option>
                </select>
			...
			<tbody>
				...
				<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
						...
						<td><?php echo (getCurrencynameByCurrency($vo["currency_id"])); ?></td>
							<?php if(($vo['type']) == "sell"): ?><td class="red"><?php else: ?>
						<td style="color:#039904;"><?php endif; echo (fomatOrdersType($vo["type"])); ?></td>
						...
						<td>
							...
							<!-- <a href="javascript:void(0)"  onclick="cexiao(<?php echo ($vo["orders_id"]); ?>)">撤销</a> -->

							<?php if($vo['status'] == -1): ?>--
								<?php elseif($vo['status'] == 2): ?>
									--
								<?php else: ?>
								 	<a href="javascript:void(0)"  onclick="cexiao(<?php echo ($vo["orders_id"]); ?>)">
        ...
        <div class="page" style=" min-width:200px !important; float:right;"><?php echo ($page); ?></div>
	</div>
		<div class="clear"></div>
	</div>
</div>
