	...
	protected  function cancelOrdersByOrderId($one_order){
	    ...
	    switch ($one_order['type']){
	        case 'buy':
        	   ...
        	   //褚天恩2017.4.19修改判断
        	   //如果返回的钱大于冻结资金  修改冻结资金的为0
        	   $forzen_num = M('Member')->field('forzen_rmb')->where(array('member_id'=>$one_order['member_id']))->find();
        	   $money_min = min($money,$forzen_num['forzen_rmb']);
        	   //减冻结资金
	           $r[]=$this->setUserMoney($one_order['member_id'], $one_order['currency_trade_id'], $money_min, 'dec', 'forzen_num');
	           break;
	    case 'sell':
